<#
    ...
    Program.cs calls AddWindowsService, which selects WindowsServiceLifetime when
    launched by the Windows Service Control Manager and keeps normal console
    lifetime when run interactively.

    The service prefers the framework-dependent executable generated by publish
    (`WinBridgeApi.exe`). If it is unavailable, it falls back to `dotnet <dll>`.
    It never uses `dotnet run`:
...
# --- Resolve published executable ---
$exePath = Join-Path $PublishDir "WinBridgeApi.exe"
$dllPath = Join-Path $PublishDir "WinBridgeApi.dll"

if (Test-Path $exePath) {
    # The standard deployment path has no spaces, which also avoids quoting
    # incompatibilities in sc.exe on legacy Windows PowerShell versions.
    $binPath = $exePath
    ...
    if (-not (Test-Path $DotnetPath)) {
        Write-Error "dotnet.exe not found at '$DotnetPath'. Install .NET 8 or pass -DotnetPath explicitly."
        exit 1
    }

    $binPath = '"' + $DotnetPath + '" "' + $dllPath + '"'
} else {
    Write-Error "Published executable not found in '$PublishDir'. Run 'dotnet publish -c Release -o `"$PublishDir`"' first."
    exit 1
}
...
sc.exe description $ServiceName "ASP.NET Core 8 bridge to SQL Server (SERVIDOR\SQLSERVERDAP). Loopback HTTP only; accessed through an SSH tunnel."
